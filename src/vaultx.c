...
// Write search benchmark results to CSV file
static int write_search_benchmark_csv(
  ...
  for (current_file = 1; current_file <= num_plots_to_generate;
    ...
    if (!BENCHMARK) {
      if (SEARCH) {
        ...
        printf("Threads (Hash/Sort)         : %d\n", num_threads);
        printf("Threads (I/O)               : %d\n", num_threads_io);
      ...
      if (!ok) {
        ...
        if (BENCHMARK) {
          write_search_benchmark_csv(
              ...
              io_threads,
              hash_threads,
              0,  // keep_open not applicable to single search
