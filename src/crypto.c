...
void generate_plot_id(uint8_t *plot_id_out) {
  // NOTE: we could also generate a priv key and derive a pub key from it. But
  // why?
  ...
  // secp256k1_ec_pubkey_serialize(ctx, pubkey_compressed, &public_key_len,
  // &pubkey, SECP256K1_EC_COMPRESSED);

  // concat public and private keys
  // uint8_t combined[65];
  // memcpy(combined, pubkey_compressed, 33);
  // memcpy(combined + 32, private_key, 32);
...
//     secp256k1_context_create(SECP256K1_CONTEXT_SIGN); secp256k1_pubkey
//     pubkey;
