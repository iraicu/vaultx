...
void shuffle_table2(FILE *fd_src, FILE *fd_dest, size_t buffer_size,
  ...
  MemoTable2Record *buffer_table2 =
      (MemoTable2Record *)malloc(buffer_size * sizeof(MemoTable2Record));
...
#pragma omp parallel for schedule(static)
    for (unsigned long long r = 0; r < rounds; r++) {
      ...
      off_t offset_src =
          ((r * total_buckets + i) * num_records_in_bucket) *
      ...
      size_t records_read = fread(&buffer_table2[index],
                                  sizeof(MemoTable2Record), records_per_batch,
                                  fd_src);
      ...
      off_t offset_dest =
          i * num_records_in_shuffled_bucket * sizeof(MemoTable2Record);
      if (DEBUG)
        printf("write data: offset_dest=%jd bytes=%llu\n",
               (intmax_t)offset_dest,
               num_records_in_shuffled_bucket * sizeof(MemoTable2Record) *
                   num_buckets_to_read);
...
#pragma omp parallel for schedule(static)
    ...
    size_t elementsWritten =
        fwrite(buffer_table2_shuffled, sizeof(MemoTable2Record),
               num_records_in_bucket * num_buckets_to_read * rounds, fd_dest);
    ...
    elapsed_time_io_total += elapsed_time_io;
    throughput_io =
        (num_records_in_bucket * num_buckets_to_read * rounds *
         sizeof(MemoTable2Record)) /
