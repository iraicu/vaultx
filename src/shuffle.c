...
void shuffle_table2(FILE *fd_src, FILE *fd_dest, size_t buffer_size,
                    size_t records_per_batch,
                    unsigned long long num_buckets_to_read) {
  // Calculate maximum rounds we can fit in memory
  size_t max_rounds_in_memory = buffer_size / records_per_batch;
  if (max_rounds_in_memory == 0)
    max_rounds_in_memory = 1;
  if (max_rounds_in_memory > rounds)
    max_rounds_in_memory = rounds;

  // Actual buffer size for the chunk we can process
  size_t chunk_buffer_size = max_rounds_in_memory * records_per_batch;

  if (!BENCHMARK) {
    printf("Shuffle chunking: processing %zu rounds at a time (max memory: %zu "
           "records)\n",
  ...
  MemoTable2Record *buffer_table2 =
      (MemoTable2Record *)malloc(chunk_buffer_size * sizeof(MemoTable2Record));
  ...
  if (DEBUG)
    printf("allocating %zu bytes for bufferShuffled\n",
           chunk_buffer_size * sizeof(MemoTable2Record));
  MemoTable2Record *buffer_table2_shuffled =
      (MemoTable2Record *)malloc(chunk_buffer_size * sizeof(MemoTable2Record));
  if (buffer_table2_shuffled == NULL) {
    fprintf(stderr,
            "Error allocating memory for bufferShuffled. Requested: %zu MB\n",
            (chunk_buffer_size * sizeof(MemoTable2Record)) / (1024 * 1024));
    free(buffer_table2);
    exit(EXIT_FAILURE);
  }
  ...
  for (unsigned long long i = 0; i < total_buckets;
       i = i + num_buckets_to_read) {
    // Process rounds in chunks that fit in memory
    for (unsigned long long round_chunk_start = 0; round_chunk_start < rounds;
         round_chunk_start += max_rounds_in_memory) {
      ...
      if (DEBUG) {
        printf("Processing bucket group %llu-%llu, round chunk %llu-%llu (%llu "
               "rounds)\n",
               i, i + num_buckets_to_read - 1, round_chunk_start,
               round_chunk_start + rounds_in_this_chunk - 1,
...
#pragma omp parallel for schedule(static)
      for (unsigned long long r = 0; r < rounds_in_this_chunk; r++) {
        ...
        off_t offset_src =
            ((actual_round * total_buckets + i) * num_records_in_bucket) *
            sizeof(MemoTable2Record);
        if (DEBUG)
          printf("read data: offset_src=%jd bytes=%zu\n", (intmax_t)offset_src,
                 records_per_batch * sizeof(MemoTable2Record));
        ...
        size_t records_read =
            fread(&buffer_table2[index], sizeof(MemoTable2Record),
                  records_per_batch, fd_src);
        if (records_read != records_per_batch) {
          fprintf(stderr,
                  "Error reading file, records read %zu instead of %zu\n",
      ...
      for (size_t k = 0; k < chunk_total_records; k++) {
        ...
        if (!is_nonce_nonzero(buffer_table2[k].nonce1, NONCE_SIZE) &&
          ...
          if (!BENCHMARK) {
            fprintf(stderr,
                    "Warning: Record with one zero nonce at index %zu\n", k);
...
#pragma omp parallel for schedule(static)
      for (unsigned long long s = 0; s < num_buckets_to_read; s++) {
        for (unsigned long long r = 0; r < rounds_in_this_chunk; r++) {
          off_t index_src =
              ((r * num_buckets_to_read + s) * num_records_in_bucket);
          off_t index_dest =
              (s * rounds_in_this_chunk + r) * num_records_in_bucket;

          memcpy(&buffer_table2_shuffled[index_dest], &buffer_table2[index_src],
      ...
      elapsed_time_io = end_time_io - start_time_io;
      elapsed_time_io_total += elapsed_time_io;
      throughput_io = (num_records_in_bucket * num_buckets_to_read *
                       rounds_in_this_chunk * sizeof(MemoTable2Record)) /
                      (elapsed_time_io * 1024 * 1024);

      if (!BENCHMARK)
        printf("[%.2f] Shuffle Table2 %.2f%% (chunk %llu/%llu): %.2f MB/s\n",
               omp_get_wtime() - start_time, (i + 1) * 100.0 / total_buckets,
  ...
  if (!BENCHMARK) {
    printf("Zero unshuffled nonces found: %llu out of %llu total records "
           "processed.\n",
