...
size_t writeBucketToDiskSequential(const Bucket *bucket, FILE *fd) {
  ...
  size_t elementsWritten =
      fwrite(bucket->records, sizeof(MemoRecord), num_records_in_bucket, fd);
  if (elementsWritten != num_records_in_bucket) {
    fprintf(stderr,
            "Error writing bucket to file; elements written %zu when expected "
            "%llu\n",
...
size_t count_zero_memo_records(const char *filename) {
  ...
  while ((records_read = fread(buffer, sizeof(MemoRecord), BATCH_SIZE, file)) >
         0) {
  ...
  printf("total_zero_records=%zu total_nonzero_records=%zu efficiency=%.2f%%\n",
         total_zero_records, total_nonzero_records,
         total_nonzero_records * 100.0 /
             (total_zero_records + total_nonzero_records));
