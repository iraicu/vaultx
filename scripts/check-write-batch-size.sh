...
for disk in "${disks[@]}"; do
    ...
    data_file="data/$hostname-$disk_type-write-batch-size-check1.txt"
    ...
    #    echo "Running with WRITE_BATCH_SIZE=$W"
        for i in {1..3}; do
            ...
            ./vaultx -v true -a for -f "$disk/varvara/plot/" -g "$disk/varvara/plot/" -j "$disk/varvara/plot/" -t 64 -K 29 -m 512 -W 1 -R 1 -x true >> "$data_file"
        done
    #done
