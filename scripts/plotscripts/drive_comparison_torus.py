#!/usr/bin/env python3
"""
Drive comparison for Torus (Madmax): time, power, and peak memory vs thread count.
NVME = ssd-raid0, HDD = nfs_hdd.
...
MADMAX_DIR   = os.path.join(SCRIPT_DIR, "..", "..", "newexperiments", "torus", "madmax")
...
def load_torus():
    nvme = pd.read_csv(os.path.join(MADMAX_DIR, "varying_threads_k32_madmax_nvme.csv"))
    hdd  = pd.read_csv(os.path.join(MADMAX_DIR, "varying_threads_k32_madmax.csv"))
...
def main():
    ...
    mem_nvme = nvme["peak_memory(MB)"].values.astype(float) / 1024.0
    mem_hdd  = hdd["peak_memory(MB)"].values.astype(float) / 1024.0
    ...
    fig.suptitle("Torus: Madmax k32 (HDD vs NVME)",
                 fontsize=13, fontweight="bold", y=0.995)

    out = os.path.join(IMAGES_DIR, "drive_comparison_torus.png")
...
if __name__ == "__main__":
    print("Generating Torus drive comparison …")
    main()
    print("Done.")
