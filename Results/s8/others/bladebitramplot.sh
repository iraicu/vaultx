...
# =============================================================================
# BLADEBIT RAMPLOT — VARYING THREADS
# Runs ramplot once per thread count in THREAD_COUNTS.
# Logs power and peak memory per run; appends all results to one CSV.
# Clears cache before and after each run; moves plots to Ceph.
# =============================================================================
...
# --- OUTPUT DIRECTORY (change this to redirect where .plot files are written) ---
...
if [ ! -f "$CSV_FILE" ]; then
    echo "run,threads,plot_out_dir,total_plot_time(s),total_plot_time(min),min_power(W),max_power(W),avg_power(W),total_energy(Wh),total_energy(kWh),peak_memory(MB),exit_code" \
        > "$CSV_FILE"
fi
...
echo "=============================================="
echo " BLADEBIT RAMPLOT — VARYING THREADS"
...
for N_THREADS in "${THREAD_COUNTS[@]}"; do
    ...
    LOG_FILE="$LOG_DIR/ramplot_run${RUN_NUM}_t${N_THREADS}_${TIMESTAMP}.txt"
    ...
    {
        echo "=============================="
        echo "BLADEBIT RAMPLOT RUN $RUN_NUM / $TOTAL_RUNS"
        echo "Date/Time   : $(date)"
        echo "Threads     : $N_THREADS"
        echo "Output dir  : $PLOT_OUT_DIR"
        echo "=============================="
    ...
    echo "${RUN_NUM},${N_THREADS},${PLOT_OUT_DIR},${EXP_DURATION},${DURATION_MIN},${MIN_W},${MAX_W},${AVG_W},${TOTAL_WH},${TOTAL_KWH},${PEAK_MEM_MB},${PLOT_EXIT_CODE}" \
        >> "$CSV_FILE"
